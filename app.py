...
# --- CUSTOM THEME STYLING & HIGH-VISIBILITY BUTTON ---
st.markdown(
    """
    <style>
...
# ==========================================================
# 🎨 LEFT SIDE: MULTI-LAYER TERRAIN ROUTING CANVAS
# ==========================================================
with col1:
    st.header("📍 Campus Energy Map")
    st.caption("Real-time localized routing path configuration grid with terrain tracking")
    
    # 1. EXPANDED CAMPUS LANDMARK DATABASE
    locations_db = {
    ...
    # 2. Interactive Route Configuration Dropdowns
    st.subheader("⚙️ Route Configuration")
    col_from, col_to = st.columns(2)
    
    with col_from:
        start_loc = st.selectbox("Departure Hub", list(locations_db.keys()), index=1)
    with col_to:
        end_loc = st.selectbox("Destination Hub", list(locations_db.keys()), index=14)
    ...
    # Layer A: Red Hub Dots (Properly colored and sized down to radius 8)
    hub_layer = pdk.Layer(
        "ScatterplotLayer",
        data=points_data,
    ...
    # Layer B: Laser Blue Connectivity Line (Explicit colors restored)
    ...
    # Layer C: Bold Labels Over Buildings (Set to high contrast blue/black)
    text_layer = pdk.Layer(
        ...
        get_text="name",
        get_size=20,
        get_color="[0, 30, 90, 255]",
        get_alignment_baseline="'bottom'",
        get_pixel_offset="[0, -15]"
    )
    
    # Render using the open-source street terrain layer
